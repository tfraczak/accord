...
class MessageForm extends Component {
  ...
  handleSubmit(e) {
    ...
    if (this.state.body) {
      ...
      switch(action) {
      case 'create':
        sub.speak(({ message: toSnakeCase(message) }), sub);
        this.setState({ body: '' });
        break;
      case 'update':
        sub.update(({ message: toSnakeCase(message) }), sub);
        setState({ updateMsgId: null });
        break;
      }
    }
  }
        ...
        <form
          ...
          { action === 'update' ? (
            <div className="um-options">
              escape to&nbsp;
              <button type="button" onClick={this.handleCancel} className="um-btn cancel">
                cancel
              </button>
              &nbsp;• enter to&nbsp;
